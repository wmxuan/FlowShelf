...
### 10.8 用户操作链路流程图（基础模式 vs AI 模式）

> 按用户操作顺序绘制完整链路，标注每一步在两种模式下的差异。

#### 链路 1：Bookmarklet 收藏

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：在任意网页点击 Bookmarklet 书签                                    │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────┐
│  前端: /bookmarklet?page.tsx             │
│  1. 从 URL 参数取 ?url=                  │
│  2. 从 opener 取预提取正文（3s超时）      │
│  3. POST /api/learning                   │
│     body: {                              │
│       source_url: url,                   │
│       title: '',                         │
│       item_type: 'unspecified',  ← 关键！│
│       content: 正文                      │
│     }                                    │
└────────┬────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  后端: LearningService.create_item()                                    │
│  1. 60s 同 URL 去重检测                                                 │
│  2. 入库 LearningItem (is_ready=False, item_type='unspecified')         │
│  3. item_type == 'unspecified'                                          │
│     → 跳过后台 AI 补全！等用户在暂存区选类型后再生成                      │
│     → ★ 两种模式行为一致（都不调AI）                                     │
│  4. 返回 LearningItem 给前端                                            │
└────────┬────────────────────────────────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────┐
...
│  ★ 基础/AI 模式：流程完全一致          │
│  ★ 数据落库: LearningItem 表          │
│    is_ready=False (AI未补全)          │
│    ai_summary=NULL, ai_tags=[]       │
└──────────────────────────────────────┘
```

**结论**：Bookmarklet 链路在两种模式下完全一致——都写暂存区，都不调AI（因为 item_type=unspecified）。
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：在浏览器地址栏点 ⭐ 收藏当前网页                                   │
│  触发：扩展 background/index.ts → chrome.bookmarks.onCreated 监听           │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────┐
│  扩展 Background:                                    │
│  1. 去抖 3s（避免快速连点）                           │
│  2. 调 content script 提取页面正文                    │
│  3. POST /api/learning                               │
│     body: {                                          │
│       source_url: tab.url,                           │
│       title: tab.title,                              │
│       item_type: 'unspecified',  ← 与bookmarklet同   │
│       content: 正文                                  │
│     }                                                │
└────────┬────────────────────────────────────────────┘
         │
         ▼
┌──────────────────────────────────────────────────────────┐
│  后端: 与 Bookmarklet 同一链路                           │
│  LearningItem 入库，item_type='unspecified'，不调AI      │
│                                                          │
│  ★ 基础/AI 模式：流程完全一致                             │
└──────────────────────────────────────────────────────────┘
```

**结论**：⭐ 按钮与 Bookmarklet 行为一致——都写暂存区，item_type=unspecified，不调AI。
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：点击扩展图标 → Popup 弹出 → 点击"收藏"按钮                        │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────────┐
│  扩展 Popup:                                                        │
│  1. Content Script 提取正文                                         │
│  2. POST /api/classify  ← 先AI分流！                                │
...
└────────┬────────────────────────────────────────────────────────────┘
         ...
         ▼                                                         ▼
┌──────────────────────────────┐   ┌──────────────────────────────────────────┐
│  基础模式: DemoAIProvider     │   │  AI 模式: RealAIProvider                  │
│  classify_tool():            │   │  classify_tool():                         │
│  URL关键词匹配:              │   │  LLM 分析:                                │
│  "tool/dashboard" → tool    │   │  分析内容语义 → article/tool/video         │
│  "video/youtube" → video    │   │  + 3-5 个语义标签                         │
│  其余 → article             │   │                                            │
│  tags: ["文章","待学习"]     │   │                                            │
└────────┬─────────────────────┘   └────────┬─────────────────────────────────┘
         │                                    │
         │  返回 {type, tags}                  │  返回 {type, tags}
         │                                    │
         ▼                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│  Popup: 显示分流结果让用户确认/修正                                   │
│  用户点击"确认收藏"                                                   │
│                                                                      │
│  POST /api/learning                                                  │
│  body: { source_url, title, item_type=确认的类型, content }          │
│  ★ item_type 明确（article 或 tool）→ 触发后台 AI 补全！             │
└────────┬────────────────────────────────────────────────────────────┘
         │
         ├──── AI 模式 ────────────────────────────────────────────┐
         │                                                         │
         ▼                                                         ▼
┌──────────────────────────────────┐  ┌────────────────────────────────────┐
│  基础模式: _ai_enrich()          │  │  AI 模式: _ai_enrich()             │
│  DemoAIProvider.generate_card()  │  │  RealAIProvider.generate_card()    │
│  → 固定模板摘要/观点/标签        │  │  → LLM 生成真实摘要/观点/标签      │
│  DemoAIProvider.generate_tool()  │  │  RealAIProvider.generate_tool()    │
│  → 固定模板描述/标签             │  │  → LLM 生成真实描述/标签           │
│  Embedding: hash向量             │  │  Embedding: bge本地向量            │
│                                  │  │                                    │
│  ❌ 写入无意义mock数据            │  │  ✅ 写入有意义的AI生成数据          │
└────────┬─────────────────────────┘  └────────┬──────────────────────────┘
         │                                        │
         ▼                                        ▼
┌─────────────────────────────────────────────────────────────┐
│  落库: LearningItem 表                                      │
│  is_ready=True, ai_summary/ai_tags/embedding 已填充         │
│  用户可在 /learning 页看到并执行"生成卡片"/"加入工具箱"      │
└─────────────────────────────────────────────────────────────┘
```

**关键差异**：Popup 收藏是**唯一一条在基础模式下就触发AI的链路**（因为 item_type 明确），而 Bookmarklet/⭐ 的 item_type=unspecified 不调AI。
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：/learning 页 → 点击"生成知识卡片"或"加入工具箱"                    │
│  API: POST /api/learning/{id}/convert                                       │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ├──── item_type=article ──────────────┐
         │                                     │
         ▼                                     ▼
┌────────────────────────────────────┐  ┌────────────────────────────────────┐
│  转卡: LearningService.convert()   │  │  入工具箱:                         │
│  1. 若 is_ready=False → 先AI补全  │  │  1. 若 is_ready=False → 先AI补全   │
│  2. 用AI数据创建 Card 记录         │  │  2. 用AI数据创建 Tool 记录          │
│  3. Card 入库 (cards 表)           │  │  3. Tool 入库 (tools 表)           │
│  4. 标记 is_converted=True         │  │  4. 标记 is_converted=True         │
└────────┬───────────────────────────┘  └────────┬───────────────────────────┘
         │                                            │
         ├──── AI 模式 ──────────────────────────────┤
         │                                            │
...
┌──────────────────────────────────────┐  ┌──────────────────────────────────────┐
│  基础模式:                           │  │  AI 模式:                            │
│  Card.summary = "这是对文章《...》    │  │  Card.summary = LLM真实摘要         │
│    的摘要。文章主要讨论了..."          │  │  Card.key_points = LLM真实观点      │
│  Card.key_points = ["核心观点1/2/3"] │  │  Card.ai_tags = 语义标签             │
│  Card.ai_tags = ["技术","架构",      │  │  Card.embedding = bge向量            │
│    "最佳实践"]                        │  │                                      │
│  Card.embedding = hash向量           │  │  ✅ 高质量数据，可语义搜索            │
│  ❌ 无意义数据，污染知识库            │  │                                      │
└──────────────────────────────────────┘  └──────────────────────────────────────┘
         │                                            │
         ▼                                            ▼
┌─────────────────────────────────────────────────────────────┐
│  落库位置:                                                   │
│  卡片 → cards 表 (id, source_url, title, ai_summary,         │
│                    key_points, ai_tags, embedding)           │
│  工具 → tools 表 (id, url, title, description, ai_tags,     │
│                    embedding, visit_count)                   │
│                                                              │
│  ⚠️ 基础模式落库的数据：summary/tags 无意义，embedding 不可搜  │
└─────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：打开新标签页 → 扩展 newtab 重定向 → /tabs                         │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────┐
│  /tabs/page.tsx 首先检测 Chrome Bridge 是否可用              │
│  checkBridgeAvailable(5s) → 二次重试                         │
└────────┬────────────────────────────────────────────────────┘
         │
         ├──── Bridge 不可用（未安装扩展）──────────────────────────┐
         │                                                         │
         ▼                                                         ▼
┌──────────────────────────────────────┐  ┌──────────────────────────────────────┐
│  Bridge 可用                         │  │  Bridge 不可用                        │
│  ↓                                   │  │  ↓                                    │
│  1. getAllTabs() 获取当前窗口所有 Tab  │  │  显示引导提示：                        │
│  2. 过滤 HTTP/HTTPS 标签             │  │  "需要安装 FlowShelf 浏览器扩展"       │
│  3. 检查 Chrome 原生群组              │  │  + 安装步骤说明                        │
│     有群组 → 按群组分组               │  │                                        │
│     无群组 → 按窗口分组               │  │  ★ 基础/AI 模式行为一致               │
│  4. 展示分组卡片                      │  │  （此步骤不涉及AI）                    │
│                                      │  │                                        │
│  ★ 基础/AI 模式：初始加载行为一致      │  └──────────────────────────────────────┘
└────────┬─────────────────────────────┘
         │
         ▼  用户操作：点击"✏️ 编辑分组" → 进入编辑模式
┌─────────────────────────────────────────────────────────────────────────────┐
│  编辑模式下可用功能：                                                        │
│  ✅ 拖拽 Tab 跨组移动 / 组内排序                                             │
│  ✅ 组名编辑 / 新建分组 / 删除空分组                                          │
│  ✅ "一键同步" → 应用为 Chrome 原生群组                                       │
│  ✅ "🤖 AI智能分组" → POST /api/tabs/group                                  │
│  ✅ 每个 Tab 行: [📄 卡片] [🔧 工具] [✕ 关闭]                                │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ├──── 用户点 "🤖 AI智能分组" ──────────────────────────────┐
...
│  按域名+路径前缀分组               │  │  LLM 语义聚类分组                   │
│  例:                              │  │  例:                               │
│  github.com · 问答                │  │  "前端开发相关"                     │
│  github.com · 代码协作             │  │  "Python 学习资源"                  │
│  stackoverflow.com 相关            │  │  "工作工具"                         │
│                                   │  │                                     │
│  ✅ 降级可用（域名分组有基本合理性） │  │  ✅ 语义聚类更智能                   │
└───────────────────────────────────┘  └────────────────────────────────────┘
         │                                                         │
         ├──── 用户点 [📄 卡片] 或 [🔧 工具] ─────────────────────┤
         │                                                         │
         ▼                                                         ▼
┌───────────────────────────────────────────────────────────────────────┐
│  handleCollectToLearning():                                           │
│  1. getTabContent(tab.id) → 提取正文                                  │
│  2. POST /api/learning { source_url, title, item_type, content }      │
│     item_type 由按钮决定: 📄→article, 🔧→tool                         │
│  3. ★ item_type 明确 → 触发后台 AI 补全                               │
│     基础模式: mock摘要 + hash向量                                      │
│     AI模式: 真实摘要 + bge向量                                         │
│  4. 落库: LearningItem 表 → 可在 /learning 查看                       │
└───────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  用户操作：Header 搜索框输入 → /search?q=xxx 或直接在 /search 页搜索         │
└────────┬────────────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────────┐
│  前端 /search/page.tsx:                                             │
│  并行调用两次 searchApi.semantic():                                  │
│    POST /api/search { query, type='cards', limit }                  │
│    POST /api/search { query, type='tools', limit }                  │
│  合并后按 score 降序                                                 │
└────────┬────────────────────────────────────────────────────────────┘
         │
         ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  后端 SearchService.semantic_search():                                  │
│  1. _get_query_embedding(query, is_query=True)                         │
│     生成查询向量 → 用于向量检索                                         │
│  2. 遍历所有 Card/Tool → _compute_hybrid_score()                       │
│     score = 向量相似度 × 0.7 + 关键词分数 × 0.3                         │
│  3. 过滤 score > 0 的结果，排序截断                                     │
└────────┬────────────────────────────────────────────────────────────────┘
         │
         ├──── AI 模式 ────────────────────────────────────────────┐
         │                                                         │
...
│  DemoAIProvider.generate_embedding() │  │  bge-small-zh-v1.5 本地模型          │
│  → hash向量 (md5伪向量)              │  │  → 真实语义向量                      │
│                                      │  │                                      │
│  Doc Embedding (已存数据):            │  │  Doc Embedding (已存数据):            │
│  全部是 hash 向量                     │  │  全部是 bge 向量                     │
│                                      │  │                                      │
│  向量相似度计算:                      │  │  向量相似度计算:                      │
│  hash向量之间无语义关系               │  │  余弦相似度有语义含义                 │
│  "React教程" 和 "React学习指南"       │  │  "React教程" 能搜到                  │
│  向量完全不相关 → 搜不到              │  │  "React学习指南" ✅                   │
│                                      │  │                                      │
│  降级到关键词分数:                    │  │  混合分数:                           │
│  score ≈ keyword × 0.3              │  │  score = vec×0.7 + keyword×0.3      │
│  ❌ 仅靠关键词，无语义能力             │  │  ✅ 语义+关键词双保险                 │
└──────────────────────────────────────┘  └──────────────────────────────────────┘
```
...
│  Header 导航栏                                                       │
│                                                                      │
│  基础模式 (aiMode=false):          AI 模式 (aiMode=true):            │
│  ┌──────────────────────────┐     ┌──────────────────────────────┐   │
│  │ 🗂️ Tab 管理  (alwaysShow)│     │ 🗂️ Tab 管理                  │   │
│  │ 📥 暂存区   (alwaysShow)│     │ 📚 卡片库                    │   │
│  │                           │     │ 🛠️ 工具箱                    │   │
│  │ ❌ 卡片库  (隐藏)        │     │ 📥 暂存区                    │   │
│  │ ❌ 工具箱  (隐藏)        │     │ 🔍 全局搜索                  │   │
...
│                                                                      │
│  ⚠️ 但 URL 直达仍可用：                                             │
│  /cards   → 页面加载正常，但数据含mock摘要                            │
│  /toolbox → 页面加载正常，但数据含mock描述                            │
│  /search  → 页面加载正常，但搜索无语义能力                            │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
          ...
          ▼                ▼                ▼
   ┌─────────────┐  ┌───────────┐  ┌───────────────┐
   │ Bookmarklet  │  │  ⭐ 星号   │  │ 扩展 Popup    │
   │ /bookmarklet │  │  书签双写  │  │ 快速收藏      │
   └──────┬──────┘  └─────┬─────┘  └───────┬───────┘
          │               │                 │
          │               │          ┌──────┴──────┐
          │               │          │ POST /api/  │
          ...
          ▼               ▼                 ▼
   ┌───────────────────────────────────────────────┐
   │        POST /api/learning (写入暂存区)          │
   │  Bookmarklet/⭐: item_type='unspecified'      │
   │                  → 不调AI，等用户选类型         │
   │  Popup:          item_type='article'/'tool'   │
   │                  → 异步AI补全（基础=mock/AI=真实）│
   └───────────────────────┬───────────────────────┘
                           │
                           ▼
   ┌───────────────────────────────────────────────┐
